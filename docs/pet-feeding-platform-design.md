...
- 本地开发联调基址：`http://101.42.24.114`
- 体验版/正式版基址：`https://mayunfei.asia`
- 小程序接口路径本身已包含 `/api/miniapp/...`，因此基址本身不能再带 `/api`
- 微信开发者工具本地联调优先使用开发基址
- 体验版与正式版发布前，必须切回域名 HTTPS 基址
...
- 登录页支持手机号 + 密码注册与登录
- 注册时不再要求填写昵称，避免与手机号注册流程冲突
- 登录/注册失败优先展示后端返回的 `message`
- 退出登录后必须清除本地登录态，并跳转回 `pages/login/login`
...
- 微信小程序开发、调试、编译、预览、真机联调统一使用微信开发者工具
- 页面问题、编译报错、接口联调结果默认以微信开发者工具中的表现为准

### 5.2 当前联调结论

- 本地微信开发者工具联调已验证注册成功
- 开发环境因 Windows/微信开发者工具到 `https://mayunfei.asia` 的 443/TLS 握手不稳定，当前采用开发与线上双基址策略
- 服务器已临时开放 `http://101.42.24.114` 开发入口，仅用于本地微信开发者工具联调
- 该临时 HTTP 开发入口不建议长期保留在生产公网环境

### 5.3 维护要求

- 后续只要页面结构、接口路径、角色流程、部署地址、联调方式发生变化，必须同步更新本文档
