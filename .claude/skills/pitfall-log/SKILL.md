...
name: pitfall-log
description: 踩坑日志 — 记录踩过的坑、原因和解决方案。涵盖 403/500 安全链、物种映射、唯一约束、UI 输入框等 7 个主题。
---
...
**当前状态**（截至 2026-06-26 下午 15:30）：
- **后端** `:8080` 运行正常，全部 API 验证通过
- **数据库** 7 只宠物（`猫`×4 / `狗`×2 / `DOG`×1）、7 个订单、2 条评价、1 个喂养员
- **页面** 6个页面全部重做：首页/预约页/喂养员列表/喂养员详情/宠物列表/添加宠物页

---

### 2026-06-26 (追加): 喂养员详情页缺少"选择喂养员去预约"入口

**现象**：从喂养员列表点进去详情页之后，只能看评价，不能直接选中这个喂养员去下单。用户反馈"点进去就是评价"。

**原因**：`detail.wxml` 只有喂养员信息和评价列表，没有跳转到预约页的操作按钮。

**解决**：重做详情页：
- 顶部改为首字母名片卡片（头像 + 姓名 + 区域 + 星级）
- 经验和简介改为信息块
- 新增两处"选择该喂养员去预约"按钮（顶部快捷操作 + 底部绿色大按钮）
- `detail.js` 新增 `goOrder()` 方法跳转到 `/pages/orders/create/create`
- 评价列表优化为白色圆角卡片

**教训**：详情页要有明确的 CTA（Call-to-Action），用户来这里的目的是**选择喂养员去预约**，不是看评价。评价是辅助决策的信息，不是目标。应该在页面最显眼的位置放置"去预约"按钮。
- **标签** miniapp: v1.1.0 (28 commits since v1.0.0)、admin: v1.1.0 (6 commits since v1.0.0)
- **踩坑日志** 7 条记录 + 完整修复汇总，随代码仓库 `/pitfall-log` 技能携带
- **GitHub** 两个仓库 `master` 分支均已推送最新
