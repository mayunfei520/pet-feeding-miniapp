...
**当前状态**：
- 后端 :8080 运行中，所有 12 个 API 验证通过 ✅
- 数据库有 7 只宠物（包含中文 `猫`/`狗` 和英文 `CAT`/`DOG`/`OTHER` species）
- 前端物种映射表同时覆盖中英文 key，`物种标签显示原始值` 问题已彻底修复
- 添加宠物页 WXML 预览卡片中 `species === 'DOG'/'CAT'/'OTHER'` 判断未包装中文兼容（添加页 value 固定是这三个英文 code，不需要中文），颜色 emoji 正常
